...
struct PostCard: View {
    ...
    private var resolvedHandle: String? {
        if let handle = author?.handle, !handle.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return HandleFormatter.normalizedHandle(from: handle)
        }
    ...
    private func commentAuthor(for comment: Comment) -> String {
        let normalized = HandleFormatter.normalizedHandle(from: comment.authorName)
